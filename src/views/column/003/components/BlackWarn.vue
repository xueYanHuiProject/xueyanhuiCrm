<template>
    <el-dialog
            ...
            <el-form ref="form" :model="addForm" label-width="80px">
                <el-form-item label="拉黑原因">
                    <el-select v-model="addForm.region" placeholder="请选择活动区域">
                        <el-option :label="item.reasonName" :value="item.reasonType" v-for="(item,index) in blackReason" :key="index"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="拉黑回复"  class="blackWarnTextarea">
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <el-button @click="handleClose" type="danger">拉黑</el-button>
        </span>
    </el-dialog>
</template>
<script>
    ...
    const { mapGetters,mapActions } = createNamespacedHelpers('module003');
    export default {
        data(){
            ...
            return {
                ...
                blackReason:Common.blackReason()
            }
        },
        computed:{
            ...mapGetters(['blackWarn','addMessage','iconList'])
        },
        watch:{
            addForm:{
                handler(n){
        ...
        methods:{
            ...mapActions(['showBlackWarn','hideBlackWarn','createColumn','hideMsg']),
            handleClose(){
                let t = this;
                t.hideBlackWarn();
                t.addForm = {
